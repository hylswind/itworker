...
from openzp_itworker.server import actions, github
...
def test_deploy_rejects_duplicate_version():
    ssm = FakeSsm({"/openzp/apps/x": json.dumps({"owner_id": 1, "repo_id": 2}),
                   "/openzp/versions/x/abc1234": "{}"})
...
def test_app_launch_template_blocks_container_imds_and_uses_version_profile():
    ...
    actions._create_lt(ctx, priority=1, commit="abc1234", ami_id="ami-1",
                       profile_arn="arn:aws:iam::123:instance-profile/openzp-secret-1-abc1234")
    md = ec2.lt_kwargs["LaunchTemplateData"]["MetadataOptions"]
    assert md["HttpPutResponseHopLimit"] == 1 and md["HttpTokens"] == "required"
    assert ec2.lt_kwargs["LaunchTemplateData"]["IamInstanceProfile"]["Arn"].endswith("openzp-secret-1-abc1234")
...
def test_put_secret_stores_securestring():
    ...
    stored = ssm.params["/openzp/secrets/demo.dev/abc1234"]
...
def test_version_role_scoped_to_its_own_secret():
    ...
    assert name == "openzp-secret-3-abc1234"
    assert profile_arn.endswith("instance-profile/openzp-secret-3-abc1234")
    policy = next(kw["PolicyDocument"] for n, kw in iam.calls if n == "put_role_policy")
    assert "parameter/openzp/secrets/demo.dev/abc1234" in policy
...
def test_bake_component_has_dockerfile_contract():
    ...
    assert "OPENZP_VERSION_SECRET" in data
