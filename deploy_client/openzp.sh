...
#   openzp.sh https://admin.<domain> <API_KEY> deploy <app> <commit-sha>  # bake + route /{app}/{sha}/*
#   openzp.sh https://admin.<domain> <API_KEY> delete <app> <commit-sha>  # tear one version down
...
case "$ACTION" in
  ...
  deploy) body="{\"app\":\"${4:?app name}\",\"commit\":\"${5:?commit sha}\"}"; interval=30 ;;
  delete) body="{\"app\":\"${4:?app name}\",\"commit\":\"${5:?commit sha}\"}"; interval=15 ;;
...
while :; do
  doc=$(curl -fsS "$API/status?id=$job" -H "x-api-key: $KEY")
