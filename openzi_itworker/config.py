...
the prefixes here (``openzi-secret-*`` etc.)."""
...
APP_PARAM = "/openzi/apps/{app}"                    # immutable app->repo binding (no-overwrite)
VERSION_PARAM = "/openzi/versions/{app}/{commit}"   # per-version resource manifest
APP_PREFIX = APP_PARAM.split("{", 1)[0]             # "/openzi/apps/"      (recover wipes these)
VERSION_PREFIX = VERSION_PARAM.split("{", 1)[0]     # "/openzi/versions/"  (recover wipes these)
PRIORITY_COUNTER = "/openzi/priority-counter"       # monotonic ALB-priority allocator
JOB_PARAM = "/openzi/jobs/{job_id}"                 # async job status (survives restart)
...
PLATFORM_CONFIG_PARAM = "/openzi/platform-config"   # JSON of platform facts (from CFN)
API_KEY_PARAM = "/openzi/api-key"                   # SecureString; the control-plane bearer key
PINNED_COMMIT_PARAM = "/openzi/pinned-commit"       # the workflow-pinned commit (replacement boot)
REPO_PARAM = "/openzi/repo"                         # owner/name to clone (replacement boot)
CONSOLE_PASSWORD_PARAM = "/openzi/console-password"  # SecureString; billing user's login password
...
SECRET_PARAM = "/openzi/secrets/{app}/{commit}"
SECRET_ROLE_NAME = "openzi-secret-{priority}-{commit}"   # also the instance-profile name
SECRET_PREFIX = SECRET_PARAM.split("{", 1)[0]            # "/openzi/secrets/"
SECRET_ROLE_PREFIX = SECRET_ROLE_NAME.split("{", 1)[0]   # "openzi-secret-"
...
# desync them. ---
TG_NAME = "openzi-tg-{priority}-{commit}"
LT_NAME = "openzi-lt-{priority}-{commit}"
ASG_NAME = "openzi-asg-{priority}-{commit}"
TG_PREFIX = TG_NAME.split("{", 1)[0]     # "openzi-tg-"
LT_PREFIX = LT_NAME.split("{", 1)[0]     # "openzi-lt-"
ASG_PREFIX = ASG_NAME.split("{", 1)[0]   # "openzi-asg-"
...
CONTROL_ASG_NAME = "openzi-control"     # the restart SFN targets this name
CONTROL_LT_NAME = "openzi-control"
CONTROL_PORT = 8080
...
ADMIN_PROFILE_NAME = "openzi-admin"

# --- CloudFormation ---
PLATFORM_STACK_NAME = "openzi-platform"
